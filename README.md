...
`after\plugin\colors.lua` and `after\plugin\recording.lua` as they are not directly tied to any
plugins.\ `colors.lua` will apply all the settings of the selected colorscheme, `source.lua` is a
helper for any files that need to be sourced after startup, and `buffers.lua` is optional, as it can
slow down the startup time but will open, in separate buffers, every file in the specified directory
that has any of the file extensions specified - this can be useful because renaming symbols across
multiple files can fail if you need to change words in a buffer that is not already open.
